...
const ICSE = () => {
  ...
  // Fetch school data
  const fetchSchools = () => {
    axios.get('http://localhost:5000/admin/addicseBord/getall')
      .then((result) => {
  ...
  const handleStateChange = (event) => {
    ...
    const filteredByState = listing.filter(item => item.state === state);
    const uniqueDistricts = [...new Set(filteredByState.map(item => item.district).filter(Boolean))];

    setDistricts(uniqueDistricts);
    setFilteredListing(filteredByState);
  };
  ...
  const handleDistrictChange = (event) => {
    ...
    const filtered = listing.filter(item =>
      item.state === selectedState && item.district === district
    );
  ...
  return (
    <div className='relative'>
      ...
      <div className='md:ml-36 my-3'>
        <h1 className='text-3xl font-semibold text-sky-900'>ICSE Board Schools</h1>
        <div className='flex text-sky-900 gap-4 mt-4'>
          <select
            className="p-2 border border-teal-400 hover:border-fuchsia-400 pl-5 pr-5 rounded outline-none"
            value={selectedState}
          ...
          <select
            className="p-2 border border-teal-400 hover:border-fuchsia-400 outline-none pl-5 pr-5 rounded"
          ...
          <table className="w-[80%] md:mx-36 border-collapse border overflow-x-auto border-gray-200">
            ...
            <tbody>
              {filteredListing.map((item, index) => (
                <tr key={item._id} className={`h-[100px] ${index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}`}>
                  ...
                  <td className="pr-4 py-2 text-[#84c3d3] font-semibold">
                    <Link href={'/mainpages/viewUniversity/' + item._id}>
                      {item.SchoolName}
                      <h1 className='text-sm font-normal text-gray-600'>{item.SchoolAddress}</h1>
                    </Link>
                  </td>
